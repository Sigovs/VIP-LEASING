...
export function InventoryGrid({ vehicles }: { vehicles: Vehicle[] }) {
  ...
  return (
    <>
      <div className="sticky top-16 md:top-20 z-30 bg-bg/85 backdrop-blur-md border-y border-border">
      ...
      <Container className="py-10 md:py-16">
        {filtered.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6">
            {filtered.map((v, i) => (
              <VehicleCard key={v.slug} vehicle={v} variant="plate" priority={i < 4} />
            ))}
          </div>
        )}
      </Container>
